...
class CheckoutPage
    ...
    def assert_product_details(product)
        product_title = find ('.item-detalils h1')
        expect(product_title.text).to eql product[:name]

        sub_prince = find('.subtotal .sub-price')
        expect(sub_prince.text).to eql product[:price]
